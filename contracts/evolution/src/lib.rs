...
impl Evolution {
    ...
    pub fn set_evolution_params(
        ...
        if min_stake > shared::PRICE_UPPER_BOUND {
            panic!("Minimum stake exceeds safe maximum");
        }
        if cooldown_seconds > shared::MAX_AGE_SECONDS {
    ...
    pub fn get_request(env: Env, request_id: u64) -> Option<shared::EvolutionRequest> {
        if request_id == 0 {
            return None;
        }
        let request_key = Self::build_request_storage_key(&env, request_id);
